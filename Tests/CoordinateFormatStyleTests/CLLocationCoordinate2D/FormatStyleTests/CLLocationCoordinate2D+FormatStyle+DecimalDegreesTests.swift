...
struct CLLocationCoordinate2D_FormatStyle_DecimalDegrees_Tests {
    let formatStyle = CLLocationCoordinate2D.FormatStyle.DecimalDegrees()
    
    @Test func defaultArguments() {
        #expect(CLLocationCoordinate2D(latitude: 90.0, longitude: 180.0).formatted(formatStyle) == "90.0° N, 180.0° E")
        #expect(CLLocationCoordinate2D(latitude: -90.0, longitude: -180.0).formatted(formatStyle) == "90.0° S, 180.0° W")
